...
serve(async (req) => {
  // 1. Handle the "Handshake" (CORS Preflight)
  ...
  try {
    ...
    // Get the user from the JWT (Authorization header)
    ...
    if (dbError || !profile?.stripe_customer_id) {
      throw new Error("Missing Stripe Customer ID. Have you made a purchase yet?")
    }
